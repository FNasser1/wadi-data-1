...
def wadi_badge(text: str):
    html = (
        ...
        "border-radius:12px;display:inline-block;font-weight:600;\">{text}</div>"
    ).format(bg=WADI_COLORS['background'], fg=WADI_COLORS['critical'], text=text)
    st.markdown(html, unsafe_allow_html=True)

def wadi_title(text: str):
    html = "<h1 style='color:{fg};margin-bottom:0.2rem;'>{t}</h1>".format(
        fg=WADI_COLORS['critical'], t=text
    )
    st.markdown(html, unsafe_allow_html=True)

def wadi_subtitle(text: str):
    html = "<h3 style='color:{fg};margin-top:0.25rem;'>{t}</h3>".format(
        fg=WADI_COLORS['critical'], t=text
    )
    st.markdown(html, unsafe_allow_html=True)
...
def load_or_generate(days: int = 7, n_machines: int = 6, freq: str = "2min", seed: int = 1337):
    ...
    # quick processing for app responsiveness
...
def get_model(model_path: str = "rf_failure_model.joblib"):
    # Try loading an existing model; otherwise train quickly on small synthetic set
    try:
        bundle = load_model(model_path)
        return bundle
    except Exception:
        train_model(train_csv=None, model_out=model_path)
        return load_model(model_path)


# ---------------------------- TRANSLATION SHORTCUTS ----------------------------
def T(key: str) -> str:
    # Prefer locales/ files; fallback to built-ins in visualizations
    try:
        return get_text(key)
    except Exception:
...
def sidebar_controls(df: pd.DataFrame, lang: str):
    # Logo section - tries image first, falls back to text
    try:
        # Try to load logo from assets
        st.sidebar.image(
            "assets/logo.png",
            caption="Wadi Data" if lang == "en" else "وادي داتا",
...
def page_overview(df: pd.DataFrame, meta: dict, lang: str):
    ...
    with c1:
        wadi_badge(("🛠️ Machines: {n}").format(n=total_machines) if lang == "en"
                   else "🛠️ عدد الآلات: {n}".format(n=total_machines))
    with c2:
        wadi_badge(("⏱️ Last Update: {t}").format(t=last_ts.strftime('%Y-%m-%d %H:%M')) if lang == "en"
                   else "⏱️ آخر تحديث: {t}".format(t=last_ts.strftime('%Y-%m-%d %H:%M')))
    with c3:
    ...
    latest = (
        dfh.sort_values("timestamp")
           .groupby("machine_id")
           .tail(1)
           .sort_values("health", ascending=False)
    )
    st.dataframe(latest[["machine_id", "health"]].set_index("machine_id"))
...
def page_vibration(df: pd.DataFrame, machine_id: str, days: int, lang: str):
    ...
    # take the most recent N samples window for FFT
    dff = df[df["machine_id"] == machine_id].copy()
    dff["timestamp"] = pd.to_datetime(dff["timestamp"])
    dff = dff.sort_values("timestamp").tail(2048)  # small window
...
def page_predictions(df: pd.DataFrame, machine_id: str, days: int, lang: str):
    ...
    model = get_model()
    # Filter to selected machine & recent window
    dff = df[df["machine_id"] == machine_id].copy()
    dff["timestamp"] = pd.to_datetime(dff["timestamp"])
    dff = dff.sort_values("timestamp").tail(500)  # small recent batch for speed
    # Raw probs
    probs_df = predict_proba_df(model, dff).tail(1)
    if probs_df.empty:
        st.warning("No predictions were generated for this selection." if lang == "en"
                   else "لم يتم توليد تنبؤات لهذا الاختيار.")
        return
...
def page_alerts(df: pd.DataFrame, machine_id: str, days: int, lang: str):
    ...
    # Create simple synthetic alerts derived from health bands
    dff = df[df["machine_id"] == machine_id].copy()
    dff["timestamp"] = pd.to_datetime(dff["timestamp"])
    ...
    alerts_df = pd.DataFrame({
        ...
        "message": (["Check vibration levels"] * int(mask.sum())) if lang == "en"
                   else (["تحقق من مستويات الاهتزاز"] * int(mask.sum()))
    })
...
def page_settings(df: pd.DataFrame, meta: dict, lang: str):
    ...
    st.write("Upload CSV to replace the synthetic data" if lang == "en"
             else "ارفع ملف CSV لاستبدال البيانات الاصطناعية")
